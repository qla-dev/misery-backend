...
@if($canEnhanceArtwork)<form method="post" action="{{ route('cms.cards.enhance-artwork',$card) }}" style="margin-top:10px">@csrf<button class="secondary" type="submit" style="width:100%">Enhance with Gemini</button><p class="hint" style="margin:8px 0 0">Uses OPENROUTER_IMAGE_MODEL to restore pixelated artwork, then optimizes the JPEG below 100 KB.</p></form>@endif</section>
