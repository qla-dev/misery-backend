...
<div><section class="panel"><h3 style="margin-top:0">Raster illustration</h3><p class="hint">Current raster artwork used by the native game. Click the image to zoom.</p><div class="preview" style="margin-top:14px">@if($image)<img id="preview" src="{{ $image }}" alt="Card artwork" data-zoom-artwork>@else<div id="fallback" class="placeholder">⚡<div class="hint">Native fallback illustration</div></div><img id="preview" style="display:none" alt="Card artwork" data-zoom-artwork>@endif</div>
...
<p class="hint">Generates a high-quality square WebP with a solid black background and a strict black, white, and amber palette.</p><button type="submit">Generate WebP artwork</button></form>@else<p class="hint" style="margin-top:16px">Save the card first to enable artwork generation.</p>@endif
