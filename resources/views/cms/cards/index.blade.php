...
<td><b>{{ $card->title }}</b><div class="hint">{{ Str::limit($card->subtitle,70) }}</div></td><td>{{ number_format($card->score,2) }}</td><td><span class="badge">{{ $card->stack?->name ?? $card->deck }}</span></td>
