...
document.querySelectorAll('.enhance-card').forEach(button=>button.addEventListener('click',async()=>{
    const row=button.closest('tr'),status=row.querySelector('.enhance-status'),thumb=row.querySelector('.thumb'),zoom=row.querySelector('[data-zoom-artwork]'),exportButton=row.querySelector('.export-card'),enhancedFlag=row.querySelector('.enhanced-flag');
    button.disabled=true;button.textContent='Enhancing…';status.textContent='Working…';status.className='enhance-status';
    try{
        ...
        thumb.src=refreshedImage;if(zoom)zoom.dataset.zoomSource=refreshedImage;exportButton.dataset.image=refreshedImage;exportButton.dataset.fallback='0';enhancedFlag.hidden=false;status.textContent='Enhanced';status.classList.add('is-success');
    }catch(error){status.textContent=error.message||'Enhancement failed';status.classList.add('is-error')}
    finally{button.disabled=false;button.textContent='Enhance'}
}));
