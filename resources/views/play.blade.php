...
function render(){q('roomCode').textContent=game.code;q('players').innerHTML=game.members.map(x=>'<p>'+x.name+'</p>').join('');q('start').style.display=game.owner_id===user.id?'block':'none';if(game.started){q('room').className='hidden';q('game').className='';q('card').textContent=game.current_card?.title||'No more cards';const canFinish=game.awaiting_finish&&Number(game.current_player_id)===Number(user.id);q('answerActions').className=game.awaiting_finish?'row hidden':'row';q('finishTurn').className=canFinish?'':'hidden';q('moves').innerHTML=game.moves.map(x=>'<p>'+x.player.name+': '+(x.correct?'Correct':'Wrong')+'</p>').join('')}}
async function startGame(){try{game=await call('/games/'+game.id+'/start',{method:'POST',body:JSON.stringify({user_id:user.id})});render()}catch(e){showApiError(e)}}
...
async function finishTurn(){try{const x=await call('/games/'+game.id+'/finish-turn',{method:'POST',body:JSON.stringify({player_id:user.id})});game=x.game.data||x.game;render()}catch(e){showApiError(e)}}
