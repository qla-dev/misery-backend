...
class CardController extends Controller
{
    ...
    public function generate(Request $request, Card $card)
    {
        ...
        $prompt = implode("\n", [
            ...
            'Contained composition: build one complete foreground, middle-ground, and background scene inside a centered safe area. The environment should feel meaningful and complete, but it must not fill the canvas or extend toward its edges.',
            'Camera distance is mandatory: use a noticeably zoomed-out wide view that shows the complete scene and every object from end to end. Leave generous surrounding black negative space. Never use a close-up, tight crop, edge-to-edge composition, or oversized foreground.',
            'Composition priority: complete visibility is more important than making subjects large. Reduce the number or size of supporting details before reducing the safety margin. Keep the two people readable but comfortably contained.',
            'Mandatory safe zone: keep every person, head, hand, foot, limb, prop, hazard, object, support, effect, and meaningful environmental element completely inside the canvas with at least 12% clear pure-black padding from all four edges.',
            ...
            'Only the uniform black background may touch all four canvas edges. Do not use edge-to-edge foreground shapes or partial off-screen objects as a compositional device.',
            'Three-part composition recipe: structure every image around (1) one dominant main victim, (2) one clearly readable secondary character reacting or participating, and (3) one grounded event object or environmental cluster that explains the misery. These parts must connect naturally while remaining fully contained inside the central safe area.',
            'Primary composition reference: use the attached good-example image only for staging, visual hierarchy, grounded props, diagonal balance, and the relationship between a person and an event object. Do not copy its tight scale or proximity to the canvas edges; zoom out substantially and add the required black safety margin.',
    ...
    private function generateWithGemini(string $prompt, array $logContext): array
    {
        ...
        $parts = [
            ...
            ['text' => 'The next JPEG is a composition reference only. Borrow its visual hierarchy, grounded interaction, dimensional construction, and spatial relationship between characters and the event object. Do not copy its scale or frame coverage. Make the new scene substantially more zoomed out, with every person and object fully visible and at least 12% uninterrupted pure-black safety margin on every side. Do not copy its digits, timer, warning icon, appliance, laundry, exact viewpoint, literal content, hands-on-head pose, or one-person cast.'],
            ['inlineData' => [
                'mimeType' => 'image/jpeg',
