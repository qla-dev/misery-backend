...
class CardController extends Controller
{
    ...
    private function bosnianTranslationPrompt(string $title, string $subtitle): string
    {
        return implode("\n", [
            ...
            'Pay exceptional attention to Bosnian grammar, cases, gender, number, agreement, clitic placement, word order, idiom, and punctuation.',
            'CARD-TITLE FORM IS MANDATORY: translate the title as a natural, complete event sentence with a finite verb, normally in the active voice. Preserve the English subject and point of view explicitly. Do not turn the action into a nominal phrase or verbal noun ending in -nje, and never use passive "od strane" wording.',
            'Required example: "An Avalanche Blocks the Only Road" becomes exactly "Lavina je blokirala jedinu cestu", never "Blokada jedine ceste lavinom".',
            'Required example: "Your Dog Destroys the Wedding Cake" becomes exactly "Vaš pas je uništio svadbenu tortu", never "Uništavanje svadbene torte od strane psa".',
            'Keep title and description separate: "Fire Sprinklers Destroy an Art Exhibition" becomes "Protupožarne prskalice su uništile izložbu umjetnina". Its description becomes "Lažna dojava poplavila je svaki eksponat nekoliko minuta prije otvaranja."; never use the description translation as title_bs.',
            'For a second-person English title with an implied "you", use a natural declarative event sentence such as "Poslali ste privatnu fotografiju u porodičnu grupu", not an imperative and not "Slanje privatne fotografije u porodičnu grupu".',
            'BOSNIAN ORTHOGRAPHY IS MANDATORY: use the Bosnian Latin alphabet and preserve every required diacritic. The affricates č, ć, dž and đ must be written correctly wherever the Bosnian word requires them; š and ž are equally mandatory. Never replace these letters with c, dj, dz, s, z, or any ASCII approximation.',
            ...
            'Before returning, silently proofread every word for natural Bosnian grammar and inspect it specifically for missing or incorrect č, ć, dž, đ, š, and ž.',
    ...
    public function generate(Request $request, Card $card)
    {
        ...
        $prompt = implode("\n", [
            ...
            'Perspective variety: preserve the reference image’s dimensional, solid, three-dimensional scene construction, but do not reuse its exact camera angle or object orientation. Vary the viewpoint between left and right three-quarter views, high three-quarter views, low three-quarter views, diagonal stage views, and shallow isometric views. Never render every scene from the same perspective.',
